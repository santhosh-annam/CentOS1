...
public class MobileBase {
	...
	public AppiumDriver<MobileElement> launchMobileDriver(ExtentReports reports) throws Exception {
		...
		capabilities.setCapability(MobileCapabilityType.NEW_COMMAND_TIMEOUT, Integer.valueOf(configFileObj.getProperty("newCommandTimeout").trim()));
		...
		if(configFileObj.getProperty("app").trim().contains(".ipa")) {
			...
			if ((platformVersion != null) && (!platformVersion.isEmpty()) && (Double.valueOf(platformVersion.trim()) >= 9.3)) {
				capabilities.setCapability(MobileCapabilityType.AUTOMATION_NAME, "XCUITest");
			} else if (Double.valueOf(configFileObj.getProperty("VERSION").trim()) >= 9.3 ) {
				capabilities.setCapability(MobileCapabilityType.AUTOMATION_NAME, "XCUITest");
			} else {
		...
		String pltformVersion = sessionObj.optString("platformVersion").trim();
